...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None, show_text=True, show_ids=True, trajectories=None, k=None):
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        # Draw trajectory for the last k frames including the current frame
        if trajectories is not None and obj_id in trajectories:
            # Collect trajectory points from frame_id - k + 1 to frame_id
            recent_traj = [t[1] for t in trajectories[obj_id] if frame_id - k < t[0] <= frame_id]
...
def demo_processing(
    ...
    trajectories = {}  # Dictionary to hold trajectories for each tracklet
    with torch.no_grad():
        for frame in frames:
            if frame_id % 20 == 0:
            ...
            if len(online_tlwhs) > 0:
                online_im = plot_tracking(
                    ori_image, online_tlwhs, online_ids, frame_id=frame_id + 1, fps=1. / timer.average_time, show_text=show_text, show_ids=show_ids, trajectories=trajectories, k=k
                )
            else:
                online_im = ori_image
