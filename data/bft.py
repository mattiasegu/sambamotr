...
class BFT(MOTDataset):
    def __init__(self, config: dict, split: str, transform):
        super(BFT, self).__init__(config=config, split=split, transform=transform)

        self.config = config
        self.transform = transform
        self.dataset_name = config["DATASET"]
        assert split == "train" or split == "test", f"Split {split} is not supported!"
        self.data_dir = os.path.join(config["DATA_ROOT"], self.dataset_name)
        ...
        for vid in os.listdir(self.split_dir):
            gt_path = os.path.join(self.data_dir, "annotations_mot", split, f"{vid}.txt")
...
def build(config: dict, split: str):
    if split == "train":
        ...
        return BFT(config=config, split=split, transform=transforms_for_eval())
    else:
