...
class SambaMOTR(nn.Module):
    ...
    def forward(self, frame: NestedTensor, tracks: list[TrackInstances]):
        if self.visualize:
            os.makedirs("./outputs/visualize_tmp/sambamotr/", exist_ok=True)
        ...
        for level in range(outputs.shape[0]):
            ...
            if self.visualize:
                torch.save(reference[0, :self.n_det_queries, :].cpu(),
                           f"./outputs/visualize_tmp/sambamotr/detection_ref_pts_layer_{level}.tensor")
                torch.save(reference[0, self.n_det_queries:, :].cpu(),
                           f"./outputs/visualize_tmp/sambamotr/track_ref_pts_layer_{level}.tensor")
                torch.save(output_class[0, :self.n_det_queries, :].cpu(),
                           f"./outputs/visualize_tmp/sambamotr/detection_logits_layer_{level}.tensor")
                torch.save(output_class[0, self.n_det_queries:, :].cpu(),
                           f"./outputs/visualize_tmp/sambamotr/track_logits_layer_{level}.tensor")
                torch.save(output_bbox[0, :self.n_det_queries, :].cpu(),
                           f"./outputs/visualize_tmp/sambamotr/detection_boxes_layer_{level}.tensor")
                torch.save(output_bbox[0, self.n_det_queries:, :].cpu(),
                           f"./outputs/visualize_tmp/sambamotr/track_boxes_layer_{level}.tensor")
...
def build(config: dict):
    ...
    if config["MODEL"] == "SambaMOTR":
        ...
        ValueError(f"Unsupported model: '{config['MODEL']}'")
