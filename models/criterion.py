...
class ClipCriterion:
    def __init__(self, num_classes, matcher: HungarianMatcher, n_det_queries, aux_loss: bool, weight: dict,
                 ...
                 conv_dim: int = 4, use_dab: bool = True, use_calibration: bool = False):
        """
    ...
    def process_single_frame(self, model_outputs: dict, tracked_instances: List[TrackInstances], frame_idx: int):
        ...
        for b in range(len(tracked_instances)):
            ...
            trackinstances.hidden_state = torch.zeros((len(gt_idx), self.num_layers, self.hidden_dim * self.expand, self.state_dim), dtype=torch.float)
        ...
        if self.use_calibration:
            loss_label = self.get_loss_label_calibrated(outputs=model_outputs,
        ...
        if self.aux_loss:
            for i, aux_outputs in enumerate(model_outputs["aux_outputs"]):
                ...
                if self.use_calibration:
                    aux_loss_label = self.get_loss_label_calibrated(outputs=model_outputs["aux_outputs"][i],
                                                        gt_trackinstances=gt_trackinstances,
        ...
        for b in range(len(tracked_instances)):
            ...
            detections.hidden_state = torch.zeros((len(unmatched_indexes), self.num_layers, self.hidden_dim * self.expand, self.state_dim), dtype=torch.float)
    ...
    def get_loss_label_calibrated(self, outputs, gt_trackinstances: List[TrackInstances], idx_to_gts_idx):
        """
        Compute the classification loss with calibrated uncertainty.
        """
        pred_logits = [
            preds[~mask] for preds, mask in zip(outputs["pred_logits"], outputs["query_mask"])
        ]
        pred_boxes = [
            preds[~mask] for preds, mask in zip(outputs["pred_bboxes"], outputs["query_mask"])
        ]
        gt_labels = [
            torch.full((pred_logits[b].shape[:1]),
                    self.num_classes,
        ...
        gt_boxes = [
            t.boxes[idx_to_gts_idx[b][1][idx_to_gts_idx[b][1] >= 0]]
            for b, t in enumerate(gt_trackinstances)]
        
        aligned_ious = []
        for b in range(len(pred_logits)):
            valid_idx = idx_to_gts_idx[b][1] >= 0
            gt_labels[b][idx_to_gts_idx[b][0][valid_idx]] = gt_trackinstances[b].labels[idx_to_gts_idx[b][1][valid_idx]]

            # Compute IoUs for calibration
            ious, _ = box_iou_union(
                box_cxcywh_to_xyxy(pred_boxes[b]),
                box_cxcywh_to_xyxy(gt_boxes[b])
            )
            valid_idx = idx_to_gts_idx[b][1] >= 0
            pred_idx = idx_to_gts_idx[b][0][valid_idx]
            gt_idx = idx_to_gts_idx[b][1][valid_idx]
            aligned_iou = torch.zeros(pred_boxes[b].size(0), device=ious.device)
            aligned_iou[pred_idx] = ious[pred_idx, gt_idx]
            aligned_ious.append(aligned_iou)

        pred_logits = torch.cat(pred_logits)
        pred_boxes = torch.cat(pred_boxes)
        gt_labels = torch.cat(gt_labels)
        gt_boxes = torch.cat(gt_boxes)
        aligned_ious = torch.cat(aligned_ious)

        # Convert labels to one-hot and calibrate scores
        gt_labels_one_hot = F.one_hot(gt_labels, self.num_classes + 1)[:, :-1]\
            .to(pred_logits.dtype).to(pred_logits.device)
        gt_scores = aligned_ious.unsqueeze(-1) * gt_labels_one_hot

        # Compute the calibrated focal loss
        loss = calibrated_sigmoid_focal_loss(
            inputs=pred_logits,
            targets=gt_labels_one_hot,
            targets_score=gt_scores,
            alpha=0.25,
            gamma=2
        )
...
def calibrated_sigmoid_focal_loss(inputs, targets, targets_score, alpha: float = 0.25, gamma: float = 2):
    """
    Loss used in RT-DETR for uncertainty calibrated object detection: https://arxiv.org/abs/2304.08069.
    Args:
        ...
        targets_score: A float tensor with the same shape as inputs. Stores the
                 classification score for each element in inputs.
    ...
    pred_score = F.sigmoid(inputs).detach()
    weight = alpha * pred_score.pow(gamma) * (1 - targets) + targets_score

    loss = F.binary_cross_entropy_with_logits(inputs, targets_score, weight=weight.detach(), reduction='none')
...
def build(config: dict):
    ...
    if config["CRITERION"] == "ClipCriterion":
        ...
        return SambaClipCriterion(
            ...
            hidden_dim=config["HIDDEN_DIM"],
            use_dab=config["USE_DAB"],
            use_calibration=config["USE_CALIBRATION"] if "USE_CALIBRATION" in config else False,
