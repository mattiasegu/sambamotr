...
class BFT(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'CLASSES_TO_EVAL': ['bird'],  # Valid: ['bird']
            'BENCHMARK': 'BFT',  # Valid: 'BFT'
            ...
            'EVAL_INTERVAL': 1 # Default eval interval. If k greater than 1, evaluate every k frames.
        }
    ...
    def __init__(self, config=None):
        ...
        self.interval = self.config['EVAL_INTERVAL']
    ...
    def _get_seq_info(self):
        ...
        if self.config["SEQMAP_FILE"]:
            ...
            seqmap_file = os.path.join(self.config['GT_FOLDER'], self.gt_set + '_seqmap.txt')
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if is_gt:
            self.seq_lengths[seq] = len(read_data)
            
        # Convert data to required format
        num_timesteps = int(self.seq_lengths[seq] / self.interval)
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        MOT Challenge:
            In MOT Challenge, the 4 preproc steps are as follow:
                1) There is only one class (bird) to be evaluated, but all other classes are used for preproc.
                ...
                4) All gt dets except bird are removed, also removes bird gt dets marked with zero_marked.
        """
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            # Evaluation is ONLY valid for bird class
            if len(tracker_classes) > 0 and np.max(tracker_classes) > 1:
                raise TrackEvalException(
                    'Evaluation is only valid for bird class. Non bird class (%i) found in sequence %s at '
            ...
            # Remove gt detections marked as to remove (zero marked), and also remove gt detections not in bird class
